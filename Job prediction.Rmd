...
ggplot(df, aes(x = factor(job_level), fill = factor(attrition))) + 
  ...
  scale_fill_manual(values = c("#00BFC4", "#F8766D"), labels = c("No", "Yes"))
```
...
Based on the data exploring we can see that "environment_satisfaction", "job_satisfaction" and "relationship_satisfaction" have a significant influence on the target variable. We will combine these three variables into one called "total_satisfaction".
...
logisticmodel <- glm(attrition ~ years_at_company + total_working_years + job_involvement + business_travel + job_level + stock_option_level + over_time + years_in_current_role + monthly_income + years_at_company + marital_status + work_life_balance,
                     data = train,
                     family = binomial())
...
logistic_model_cv <- train(attrition ~ years_at_company + total_working_years + job_involvement + business_travel + job_level + stock_option_level + over_time + years_in_current_role + monthly_income + years_at_company + marital_status + work_life_balance,
                           data = train,
                           method = "glm",
                           family = binomial(),
