...
model <- glm(attrition ~ job_level_below_education, data = df_new, family = binomial())
...
# Convert job_search to a factor variable
df$attrition <- factor(data$attrition, levels = c("No", "Yes"))
...
rf_model <- randomForest(attrition ~ ., data = df, ntree = 500)
...
varImpPlot(rf_model, main = "Variable Importance for Attrition Prediction")
```
