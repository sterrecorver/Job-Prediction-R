...
df_categorical <- df %>%
  select(gender, department, education_field, job_role, marital_status) # exclude attrition

# Create dummies
dummy_model <- dummyVars(~., data = df_categorical)
data_dummies <- as.data.frame(predict(dummy_model, newdata = df_categorical))

# Combine with the response variable and make it numeric as needed for the lasso 
df_final <- cbind(df$attrition, df[, -2], data_dummies)
df_final[] <- lapply(df_final, as.numeric)

# Split the data into training and testing sets
set.seed(123)
train_index <- sample(1:nrow(df_final), 0.7*nrow(df_final))
train_data <- df_final[train_index, ]
test_data <- df_final[-train_index, ]

# Create the model matrix and response variable
x <- as.matrix(train_data[, -2]) # exclude response variable
y <- train_data[, 2] # response variable

# Fit the Lasso model with lambda = 0.1
lasso_fit1 <- glmnet(x, y, alpha = 1, lambda = 0.1)

lasso_coef1 <- coef(lasso_fit1)
print(lasso_coef1)
```
...
# Assuming your data frame is called "df"
df_categorical <- df %>%
  select(gender, department, education_field, job_role, marital_status)
...
df_new2 <- df %>% 
  select(-business_travel, -gender, -department, -education_field, -job_role, -marital_status)

# Combine with dummies and make it numeric as needed for the lasso 
df_final <- cbind(df_new2, data_dummies)
df_final[] <- lapply(df_final, as.numeric)
df_final$attrition <- df_final$attrition - 1
...
# Create the model matrix and response variable
x <- as.matrix(train_data[, -2]) # exclude attrition
y <- train_data[, 2] # attrition
...
lasso_fit2 <- glmnet(x, y, alpha = 1, lambda = best_lambda)

# Get the coefficients
lasso_coef2 <- coef(lasso_fit2)
print(lasso_coef1)

# Apply the model to the test data
test_x <- as.matrix(test_data[, -2])
test_y <- test_data[, 2]
pred_y <- predict(lasso_fit2, newx = test_x)

# Calculate the performance metrics (e.g. RMSE)
