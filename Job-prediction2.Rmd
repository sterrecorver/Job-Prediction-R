...
female_df <- subset(df, gender == "Male")
male_df <- subset(male_df, select = -c(gender))
female_df <- subset(female_df, select = -c(gender))
...
# convert probabilities to binary predictions
predictions_glm2 <- ifelse(probabilities_glm2 > 0.5, 1, 0)
...
dectreemodelcv <- train(attrition ~ .,
               data = train,
               method = "rpart",
               trControl = ctrl,
               tuneLength = 10)
...
# male
# train the random forest model using cross-validation
rf_model_male_cv <- train(attrition ~ ., data = train_male, method = "rf", trControl = train_control, ntree = 100)

# print the cross-validation results
print(rf_model_male_cv)

# predict on the test set using the cross-validated model
predictions_male_cv <- predict(rf_model_male_cv, newdata = test_male)

# evaluate the model using confusion matrix
confusionMatrix(predictions_male_cv, test_male$attrition)

# female
# train the random forest model using cross-validation
rf_model_female_cv <- train(attrition ~ ., data = train_female, method = "rf", trControl = train_control, ntree = 100)

# print the cross-validation results
print(rf_model_female_cv)

# predict on the test set using the cross-validated model
predictions_female_cv <- predict(rf_model_female_cv, newdata = test_female)

# evaluate the model using confusion matrix
confusionMatrix(predictions_female_cv, test_female$attrition)
```
...
*Random Forest on different age groups*
```{r}
# train the random forest model on the training set
...
ggplot(importance_df_4660, aes(x = reorder(feature, -importance), y = importance)) +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, size = 8),
        ...
        plot.title = element_text(size = 12, face = "bold"))
```
*Random Forest with cross-validation with different age groups*
*subgroup ages 18-30*
```{r}
# train the random forest model using cross-validation
rf_model_1830_cv <- train(attrition ~ ., data = train_1830, method = "rf", trControl = train_control, ntree = 100)

# print the cross-validation results
print(rf_model_1830_cv)

# predict on the test set using the cross-validated model
predictions_1830_cv <- predict(rf_model_1830_cv, newdata = test_1830)

# evaluate the model using confusion matrix
confusionMatrix(predictions_1830_cv, test_1830$attrition)
```

*subgroup ages 31-45*
```{r}
# train the random forest model using cross-validation
rf_model_3145_cv <- train(attrition ~ ., data = train_3145, method = "rf", trControl = train_control, ntree = 100)

# print the cross-validation results
print(rf_model_3145_cv)

# predict on the test set using the cross-validated model
predictions_3145_cv <- predict(rf_model_3145_cv, newdata = test_3145)

# evaluate the model using confusion matrix
confusionMatrix(predictions_3145_cv, test_3145$attrition)
```

*subgroup ages 46-60*
```{r}
# train the random forest model using cross-validation
rf_model_4660_cv <- train(attrition ~ ., data = train_4660, method = "rf", trControl = train_control, ntree = 100)

# print the cross-validation results
print(rf_model_4660_cv)

# predict on the test set using the cross-validated model
predictions_4660_cv <- predict(rf_model_4660_cv, newdata = test_4660)

# evaluate the model using confusion matrix
confusionMatrix(predictions_4660_cv, test_4660$attrition)
```
