...
grid.arrange(grobs = plots_c1, ncol = 2)
...
glm_model1 <- glm(attrition ~ years_in_current_role + job_role + monthly_income + job_satisfaction + work_life_balance + age + gender, 
                  data = train, 
                  family = binomial())
...
glm_model2 <- glm(attrition ~ job_level_below_education + years_in_current_role + job_role + monthly_income + job_satisfaction + work_life_balance + age + gender, 
                  data = train, 
                  family = binomial())
...
# predict on the testing set
probabilities_glm3 <- predict(glm_model3, newdata = test, type = "response")

# convert probabilities to binary predictions
predictions_glm3 <- ifelse(probabilities_glm3 > 0.5, 1, 0)
...
*Random forest with cross-validation without job_level_below_education*
```{r}
